...
const StaticDataContext = React.createContext<StaticDataContextI>({
  getData: () => new Promise((_, rej) => rej("not subscribed component!")),
});
...
const fetchData = (endpoint: string, queries: string[] = []) =>
  fetch(
    (endpoint.startsWith("http")
      ? endpoint
      : `${getOwnDomain()}/api/github/${endpoint}`) + queries.join("&")
  ).then((res) => res.json());
...
const GithubStaticDataContext: React.FC<{ children: JSX.Element }> = ({
  children,
}) => {
  const getData = <DT extends object>(
    dataName: string,
    endpoint: string = dataName,
    queries: string[] = []
  ): DT | Promise<DT> => {
